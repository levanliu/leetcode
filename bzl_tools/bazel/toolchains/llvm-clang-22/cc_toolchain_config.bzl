                    ...
                    flag_groups = [
                        flag_group(
                            flags = [
                                ...
                                "-load",
                                "-Xclang",
                                rpmRoot + "llvm-clang-22-plugin/plugins/concurrency-feature-check22.so",
                                "-Xclang",
                                "-add-plugin",
                                "-Xclang",
...
def _impl(ctx):
                ...
                flag_groups = [
                    flag_group(
                        flags = [
                            ...
                            "-L " + rpmRoot + "llvm-clang-22/lib64/x86_64-unknown-linux-gnu",
                            "-Wl,-rpath=" + rpmRoot + "llvm-clang-22/lib64/x86_64-unknown-linux-gnu"
                        ],
                    )
                ]
            ),
    ...
    if is_linux:
        ...
        ] + layering_check_features(ctx.attr.compiler, ctx.attr.extra_flags_per_feature, is_macos = False) + check_concurrency_features(ctx.attr.compiler)
    else:
        ...
        features = [
            ...
            archive_param_file_feature,
            generate_linkmap_feature,
        ] + layering_check_features(ctx.attr.compiler, ctx.attr.extra_flags_per_feature, is_macos = True) + check_concurrency_features(ctx.attr.compiler)
