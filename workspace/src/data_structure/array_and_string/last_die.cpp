...
#include <thread>
#include <vector>
namespace ns_array_last_die_thread_pool {
...
private:
    ...
    std::thread mManager;               // manager std::thread
    std::mutex mQueueMutex;             // queue std::mutex
...
public:
        ...
        mManager = std::thread([this] {
            while (!mStop) {
                ...
                if (tasksSize > workersSize && threadsRatio > 2) {
                    std::cout << "Adding new worker std::thread\n";
                    mWorkers.emplace_back([this] {
                        while (true) {
                            Task task;
                            {
                ...
                // and if the number of workers is greater than the initial number of threads
                    ...
                    std::cout << "Removing worker std::thread\n";
                    // Here we detach the std::thread so that it continues executing on its own
    ...
    ~Solution() {
        ...
        // Notify all threads and the manager std::thread to stop
        mCondVar.notify_all();
        // Wait for manager std::thread to finish
        mManager.join();
